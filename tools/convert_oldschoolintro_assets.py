...
def build_big_font(pixels: list[int], w: int, h: int) -> list[int]:
    """Replicate Scrolls88.r(): reinterpret the GIF as 56x32, write
    1 + (i%56)/7 for non-zero pixels, else 0. Output 1792 bytes."""
    if w != 7 or h < 320:
        raise SystemExit(f"fontes78.gif unexpected size {w}x{h}")
    out = [0] * 1792
    # Build the same 1D buffer Java sees (PixelGrabber returns row-major).
    flat = pixels  # already row-major
    for n in range(32):
        for n2 in range(56):
            src = n2 + n * 56
            if src >= len(flat):
                break
            v = flat[src]
            # Java: black == 0xFF000000 (-16777216). For palette images we
            # consider palette index 0 transparent and any non-zero as set.
            if v != 0:
                out[n2 + n * 56] = 1 + (n2 // 7)
    return out


def build_small_font(pixels: list[int], w: int, h: int) -> list[int]:
    """Replicate Scrolls88.p(): reinterpret 15x527 as 255x31. ac[i]=1
    for non-zero pixels else 0. Output 7905 bytes."""
    if w != 15 or h < 527:
    ...
    for n in range(31):
        for n2 in range(255):
            ...
            if src >= len(pixels):
                break
            out[n2 + n * 255] = 1 if pixels[src] != 0 else 0
    return out


def main() -> int:
    pix78, w78, h78 = read_indexed(os.path.join(ASSETS_DIR, "fontes78.gif"))
    pix15, w15, h15 = read_indexed(os.path.join(ASSETS_DIR, "fontes1517.gif"))

    big = build_big_font(pix78, w78, h78)
    small = build_small_font(pix15, w15, h15)
